        ...
        for j, electrode in enumerate(channelNumber):
            ...
            if analysisType == 1:
                ...
                if plot and analysisType == 4:
                    ...
                    plotHandles[i].plot(xs, 10 * (np.log10(np.mean(fftST, axis=0)) - np.log10(np.mean(fftBL, axis=0))),
                                        'color', plotColor)
...
def twoDguassian(grid,x_0,y_0,sigma_x,sigma_y,A=1):
    for y_pix in range(grid.shape[0]):
        for x_pix in range(grid.shape[1]):
            x = x_pix - x_0
            y = y_pix - y_0
...
def getImagePatches_forpython(rfStats, imageFileName, electrodeList, patchSizeDeg=2, viewingDistanceCM=50,
    ...
    inputImage = plt.imread(imageFileName)
    ...
    for i in range(numElectrodes):
        ...
        yCenterPos = int(np.argwhere(xAxisDeg >= -mEle)[0][0])

        if RFtype=="ellipse":
            xPosToTake=rfTMP["rfSizeAzi"]/xResDeg
            yPosToTake=rfTMP["rfSizeEle"]/yResDeg
            mask=np.zeros(shape=inputImage.shape)
            mask=cv.ellipse(mask,(xCenterPos,yCenterPos),(xPosToTake,yPosToTake),0,0,360,1,-1)
            patchData[i]=cv.bitwise_and(inputImage,mask)
        elif RFtype=="gaussian":
            xPosToTake=rfTMP["stdAzi"]/xResDeg
            yPosToTake=rfTMP["stdEle"]/yResDeg
            mask=np.zeros(shape=inputImage.shape[:-1])
            twoDguassian(mask,xCenterPos,yCenterPos,xPosToTake,yPosToTake)
            patchData[i]=cv.bitwise_and(inputImage,mask)
...
# %%
savepath = folderSourceString + "\\python_data\\extracted_data.npz"
reload = True
resave = True
[imagePatches, imageFreqs, imageSpikes] = None, None, None
if reload:
    # electrodes and spikes
    ...
    imageSpikes = plotSpikeData1Channel(plotHandles=None, channelNumber=channelNumber, stimulus_list=stimValsToUse,
                                        folderName=folderName, analysisType=2, timeVals=stRange)
    if resave:
        np.savez_compressed(savepath, imagePatches=imagePatches, imageFreqs=imageFreqs, imageSpikes=imageSpikes)

best_data = np.load(savepath, allow_pickle=True)
imagePatches = best_data['imagePatches']
imageFreqs = best_data['imageFreqs']
imageSpikes = best_data['imageSpikes']
